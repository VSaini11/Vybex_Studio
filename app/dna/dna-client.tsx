...
import {
  ...
  Coffee, RotateCcw, Moon, BarChart, Lightbulb, Users, Star
} from 'lucide-react';

import { analyzeStartupDNA, getDNAStats, submitDNAFeedback } from './actions';
...
export default function VybexDNAClient({
  initialStats,
  initialTestimonials
}: {
  initialStats: StatisticData,
  ...
  // Use initial data, no need to refetch on mount since Server Component provides it
  ...
  return (
    <main className="min-h-screen bg-black text-white selection:bg-purple-500/30">
      <div className="fixed inset-0 pointer-events-none overflow-hidden">
        <div className="absolute inset-0 bg-black mix-blend-overlay z-[-1]" />
      ...
      <div className="relative z-10 pt-32 lg:pt-40 pb-12 px-4">
        <AnimatePresence mode="wait">
          {state === 'IDLE' && (
            ...
            >
              <div className="order-2 lg:order-1 relative w-full h-[400px] lg:h-[550px] flex items-center justify-center overflow-visible">
                <div
                  className="absolute inset-0 bg-contain bg-bottom transition-all bg-no-repeat z-0 scale-110 sm:scale-[1.15] lg:scale-[1.2] origin-bottom -translate-y-2"
                  style={{
              ...
              <div className="space-y-12 order-1 lg:order-2">
                <div className="text-left">
                  ...
                  <h1 className="text-4xl lg:text-5xl font-black tracking-tight mb-3 bg-gradient-to-br from-white via-white to-purple-200 bg-clip-text text-transparent leading-[1.1]">
                    Want to know where your idea stands?
                  </h1>
                  <p className="text-purple-200/60 text-base sm:text-lg max-w-xl mb-6 lg:mb-8">
                    Then <strong className="text-white">Vybex DNA</strong> is for you. Get a data-backed report on your idea's success, competition, and execution roadmap.
                  </p>
                  ...
                  <div className="flex items-center flex-wrap gap-3 sm:gap-4 mb-8">
                    <div className="flex -space-x-2 shrink-0">
                      {testimonials.slice(0, 3).map((t, idx) => {
                        ...
                        return (
                          <div key={idx} className={`w-7 h-7 rounded-full border border-black bg-gradient-to-br ${gradients[idx % gradients.length]} flex items-center justify-center text-[9px] font-bold text-white shadow-sm`}>
                            {initials}
                          </div>
                        );
                      })}
                    </div>
                    <div className="flex items-center flex-wrap gap-x-3 gap-y-2 text-xs sm:text-[13px] text-gray-400 font-medium">
                      <span>Trusted by <strong className="text-white">{stats.foundersTrusted}+ founders</strong></span>
                      <span className="w-1 h-1 rounded-full bg-white/20 hidden sm:block" />
                      <span><strong className="text-white">{stats.totalAnalyzed.toLocaleString()}+</strong> ideas analyzed</span>
                      <span className="w-1 h-1 rounded-full bg-white/20 hidden sm:block" />
                      <span><strong className="text-white">{stats.improvedPercentage}%</strong> improved</span>
                    </div>
                  </div>
                </div>

                <div>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-4 bg-[#160b24]/40 backdrop-blur-3xl p-4 sm:p-5 rounded-[20px] border border-purple-500/10 shadow-2xl relative overflow-hidden group">
                    ...
                    <div className="relative z-10 space-y-4">
                      ...
                      <div className="pt-2">
                        <button
                          ...
                          className="w-full py-2.5 rounded-lg bg-white text-purple-950 font-black text-sm flex items-center justify-center gap-2 hover:scale-[1.02] active:scale-[0.98] transition-all disabled:opacity-30 disabled:cursor-not-allowed shadow-[0_10px_20px_rgba(255,255,255,0.05)] pt-[11px]"
                        >
                          <Zap size={18} className="fill-purple-950" />
                          Initialize Analysis
                        </button>
                      </div>
                    </div>
                ...
                <div className="order-3 lg:col-span-2 w-screen relative left-1/2 -translate-x-1/2 overflow-hidden mt-16 lg:mt-32 mb-24">
                  ...
                  <div className="max-w-7xl mx-auto px-4 mb-16 relative z-20">
                    <h3 className="text-4xl md:text-5xl font-bold text-white text-center leading-tight">
                      Trusted by Founders <br className="hidden sm:block" />
                      Everywhere
                    </h3>
                  </div>

                  <div 
                    className="flex flex-col gap-6 relative z-0"
                    style={{
                  ...
                  >
                    ...
                    <div className="flex w-max animate-marquee-left gap-6 hover:[animation-play-state:paused]">
                      {/* Duplicating the array many times to ensure seamless infinite loops */}
                      {[...testimonials, ...testimonials, ...testimonials, ...testimonials, ...testimonials, ...testimonials, ...testimonials, ...testimonials].map((t, idx) => (
                        <div 
                          key={`top-${idx}`} 
                          className="w-[380px] sm:w-[450px] bg-[#1a0b2e]/90 backdrop-blur-xl rounded-[20px] p-6 text-left flex flex-col shadow-2xl border border-purple-500/10 flex-shrink-0 cursor-pointer hover:border-purple-500/30 transition-colors"
                        >
                          <div className="flex justify-between items-start mb-5">
                            <div className="flex flex-col">
                              <p className="text-white font-bold text-[17px]">{t.name}</p>
                              <p className="text-gray-400 text-[13px]">{t.profession}</p>
                            </div>
                            <div className="w-12 h-12 rounded-full flex-shrink-0 bg-gradient-to-br from-purple-500 to-fuchsia-500 flex items-center justify-center text-white font-black text-lg shadow-[0_0_15px_rgba(168,85,247,0.3)] object-cover">
                              {t.name[0]}
                            </div>
                          </div>
                          <p className="text-purple-100/90 text-[15px] leading-relaxed font-light line-clamp-4">
                            "{t.experience || "Amazing experience building with Vybex. It entirely transformed my workflow!"}"
                          </p>
                        </div>
                      ))}
                    </div>

                    {/* Bottom Row Marquee (Right to Left) with offset starting position */}
                    <div className="flex w-max animate-marquee-right gap-6 pl-[200px] hover:[animation-play-state:paused]">
                      {[...testimonials, ...testimonials, ...testimonials, ...testimonials, ...testimonials, ...testimonials, ...testimonials, ...testimonials].reverse().map((t, idx) => (
                        <div 
                          key={`bottom-${idx}`} 
                          className="w-[380px] sm:w-[450px] bg-[#1a0b2e]/90 backdrop-blur-xl rounded-[20px] p-6 text-left flex flex-col shadow-2xl border border-purple-500/10 flex-shrink-0 cursor-pointer hover:border-purple-500/30 transition-colors"
                        >
                          ...
                          <p className="text-purple-100/90 text-[15px] leading-relaxed font-light line-clamp-4">
                            "{t.experience || "The AI features provided unmatched clarity and drastically accelerated our time to market."}"
                          </p>
                        </div>
                      ))}
                    </div>
...
function InputGroup({ label, placeholder, value, onChange }: { label: string, placeholder: string, value: string, onChange: (v: string) => void }) {
  return (
    <div className="space-y-1.5">
      <label className="text-[9px] font-bold text-gray-400 uppercase tracking-widest pl-1">{label}</label>
      <input
        type="text"
        placeholder={placeholder}
        value={value}
        onChange={(e) => onChange(e.target.value)}
        className="w-full bg-black/40 border border-white/10 rounded-md px-3 py-2 text-xs text-white placeholder:text-gray-600 focus:outline-none focus:border-purple-500/50 focus:ring-1 focus:ring-purple-500/50 transition-all"
      />
    </div>
  );
}
        ...
        <div className="lg:col-span-8 space-y-8">
          ...
          <section className="bg-white/5 border border-white/10 rounded-[24px] p-6 backdrop-blur-xl">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
              ...
              <span className={`text-[9px] font-bold px-2 py-0.5 rounded-full border ${report.competition.saturation === 'High' ? 'border-red-500/50 text-red-400 bg-red-500/5' :
        ...
        {Array.from({ length: 24 }).map((_, i) => (
          <React.Fragment key={i}>
            ...
            <motion.div
              ...
              className="absolute w-2 h-2 sm:w-3 sm:h-3 bg-purple-500 rounded-full shadow-[0_0_15px_rgba(168,85,247,0.8)]"
              style={{
                top: `${(i / 24) * 100}%`,
                left: '50%'
              }}
            ...
            <motion.div
              ...
              className="absolute w-2 h-2 sm:w-3 sm:h-3 bg-pink-500 rounded-full shadow-[0_0_15px_rgba(236,72,153,0.8)]"
              style={{
                top: `${(i / 24) * 100}%`,
                left: '50%'
              }}
...
function FeedbackForm() {
  const [formData, setFormData] = useState({ name: '', email: '', profession: '', experience: '', rating: 5, improvedIdea: true });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setSubmitting(true);
    const res = await submitDNAFeedback(formData);
    setSubmitting(false);
    if (res.success) {
      setSubmitted(true);
  ...
  if (submitted) {
    return (
      <div className="mt-12 bg-white/5 border border-white/10 rounded-[32px] p-8 text-center backdrop-blur-xl max-w-4xl mx-auto">
        <div className="mx-auto w-12 h-12 bg-green-500/20 text-green-400 rounded-full flex items-center justify-center mb-4">
          <Check size={24} />
        </div>
        <h3 className="text-xl font-bold mb-2">Feedback Received</h3>
        <p className="text-gray-400 text-sm">Thank you for helping us improve the DNA Analyzer!</p>
      </div>
    );
  }

  return (
    <div className="mt-12 bg-white/5 border border-white/10 rounded-[32px] p-8 backdrop-blur-xl max-w-4xl mx-auto">
      <div className="text-center mb-8">
        <h3 className="text-2xl font-black mb-2">Help Us Improve</h3>
        <p className="text-gray-400 text-sm">Did this report help you? Share your feedback to help us train the intelligence engine.</p>
      </div>
      <form onSubmit={handleSubmit} className="space-y-6 max-w-2xl mx-auto">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <InputGroup label="Your Name" placeholder="John Doe" value={formData.name} onChange={(v) => setFormData({ ...formData, name: v })} />
          <InputGroup label="Email Address" placeholder="john@example.com" value={formData.email} onChange={(v) => setFormData({ ...formData, email: v })} />
        </div>
        <InputGroup label="Profession" placeholder="e.g. Founder, Developer, Student" value={formData.profession} onChange={(v) => setFormData({ ...formData, profession: v })} />

        <div className="space-y-2">
          <label className="text-xs font-bold text-gray-400 uppercase tracking-widest pl-1">Share your experience</label>
          <textarea
            placeholder="How did this report help your startup idea?"
            value={formData.experience}
            onChange={(e) => setFormData({ ...formData, experience: e.target.value })}
            className="w-full bg-black/40 border border-white/10 rounded-xl px-4 py-3 text-white placeholder:text-gray-600 focus:outline-none focus:border-cyan-500/50 focus:ring-1 focus:ring-cyan-500/50 transition-all min-h-[100px]"
          />
        </div>

        <div className="space-y-2">
          <label className="text-xs font-bold text-gray-400 uppercase tracking-widest pl-1">Rating (1-5)</label>
          <div className="flex items-center gap-2">
            {[1, 2, 3, 4, 5].map((star) => (
              <button
                key={star}
                ...
                className={`p-2 rounded-xl border transition-all ${formData.rating >= star ? 'bg-yellow-500/20 text-yellow-400 border-yellow-500/50' : 'bg-white/5 text-gray-500 border-white/10 hover:border-white/30'}`}
              >
        ...
        <div className="flex items-center gap-3 bg-black/40 p-4 rounded-xl border border-white/10 cursor-pointer" onClick={() => setFormData({ ...formData, improvedIdea: !formData.improvedIdea })}>
          <input
            type="checkbox"
            id="improvedIdea"
            checked={formData.improvedIdea}
            onChange={(e) => setFormData({ ...formData, improvedIdea: e.target.checked })}
            className="w-5 h-5 accent-cyan-500 rounded bg-white/10 border-white/20 pointer-events-none"
          />
          <label htmlFor="improvedIdea" className="text-sm font-medium text-gray-300 select-none pointer-events-none">
            This analysis helped me improve or clarify my idea
          </label>
        </div>

        <button
          type="submit"
          disabled={submitting || !formData.name || !formData.email || !formData.profession}
          className="w-full py-4 rounded-xl bg-gradient-to-r from-purple-500 to-pink-500 text-white font-black text-sm uppercase tracking-widest hover:scale-[1.02] active:scale-[0.98] transition-all disabled:opacity-50 disabled:cursor-not-allowed shadow-[0_10px_20px_rgba(168,85,247,0.2)]"
        >
